...
export function InputRoot({ error = false, ...props }: InputRootProps) {
  return (
    <div
      className="bg-gray-800 h-12 group border border-gray-600 rounded-xl px-4 flex items-center gap-2 
...
export function InputIcon(props: InputIconProps) {
  return (
    <span
      ...
      group-[&:not(:has(input:placeholder-show))] group-data-[error=true]:text-danger"
      {...props}
    />
  )
}
